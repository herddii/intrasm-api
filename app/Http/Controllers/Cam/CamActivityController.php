...
class CamActivityController extends Controller
{
    ...
    public function list_tasklist(Request $request)
    {
        ...
        // return $request->tanggal;
            ...
            $var=$var->orderBy('start_date', 'DESC')->paginate(10);
            return response($var);
        } catch(\Exception $e){
    ...
    public function list_am(Request $request)
    {
        try {
            ...
            $var = \App\Models\Intrasm\User_structure::select('id as id','NAME as text','USER_ID as values')
            ->leftJoin('tbl_user as b','b.USER_ID','tbl_user_structure.staff');
            if($posisi=="AM"){
                $var=$var->where('staff', $email);
            }else if($posisi=="SGM"){
                $var=$var->where('head', $email);
            }else if($posisi=="SM"){
                $var=$var->where('manager', $email);
            }else{
                $var=$var->where('gm', $email);
            }
            return $var=$var->get();
    ...
    public function saveTask(Request $request)
    {
        ...
        $idsgm=\App\Models\Intrasm\User_structure::select('head as ID_SGM', 'manager as ID_SM', 'gm as ID_GM');
        if($posisi=="AM"){
            $idsgm=$idsgm->where('staff', $userid);
        }elseif($posisi=="SGM"){
            $idsgm=$idsgm->where('head', $userid);
        }elseif($posisi=="SM"){
            $idsgm=$idsgm->where('manager', $userid);
        }else{
            $idsgm=$idsgm->where('gm', $userid);
        }
        $idsgm=$idsgm->get();
        // return $idsgm;
        foreach($idsgm as $c){
            ...
            if(count($cost) > 0){
                foreach($cost as $k=>$v){
                    ...
                    if(isset($formtask['fileEnt'][$k])){
                        ...
                        })->save('D:jwt-firebase-intrasm/public/image/'.$safeName);
                    }
                        // end save image
    ...
    public function jumlahReport(Request $request){
        try{
            ...
            $var=\App\Models\Cam\Cam::selectRaw('COUNT(id_cam) AS jumlah_report')
            ->whereNull('deleted_at')
            ->where('type', "REPORT");
            if($posisi=="AM"){
                $var=$var->where('id_am', $email);
            }else if($posisi=="SGM"){
                $var=$var->where('id_sgm', $email);
            }else if($posisi=="SM"){
                $var=$var->where('id_sm', $email);
            }else{
                $var=$var->where('id_gm', $email);
            }
            return $var=$var->get();
    ...
    public function jumlahPlan(Request $request){
        try{
            ...
            $var=\App\Models\Cam\Cam::selectRaw('COUNT(id_cam) AS jumlah_plan')
            ->whereNull('deleted_at')
            ->where('type', "PLAN");
            if($posisi=="AM"){
                $var=$var->where('id_am', $email);
    ...
    public function accountHandling(Request $request){
        try{
            ...
            $var=\App\Models\Intrasm\User_structure::with(
                [
                    'name_ae',
                    'name_sgm',
                    'name_sm',
            ...
            )->selectRaw('id, staff, head, manager, gm, id_bu, departement')
            ->where('departement', "SLS");
            if($posisi=="AM"){
                $var=$var->where('staff', $email);
            }else if($posisi=="SGM"){
                $var=$var->where('head', $email);
    ...
    public function list_tasklist_report(Request $request)
    {
        try {
            // return $request->all();
            $userget = user($request->bearerToken());
